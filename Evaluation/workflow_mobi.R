#' @title createQualificationReportMoBi
#' @description Run a MoBi qualification workflow to create a qualification report.
#' @param qualificationRunnerFolder Folder where QualificationRunner.exe is located
#' @param mobiPortableFolder Folder where MoBi.CLI.exe is located. This is required
#' because the plan declares `"Application": "MoBi"` projects and QualificationRunner
#' dispatches per-project to MoBi.CLI.exe via its `--mobi` flag.
...
#' Only needed if any MoBi project in the plan references a PK-Sim module.
...
#' @examples
#' # Create a MoBi Qualification Report using the dev-tree defaults
#' createQualificationReportMoBi()
#'
#' # Create a MoBi Qualification Report with explicit runner/MoBi paths
#' createQualificationReportMoBi("C:/Software/QualificationRunner", "C:/Software/MoBi")
#'
#' # Create a MoBi Qualification Report and turn off creation of a doc version
#' createQualificationReportMoBi(createWordReport = FALSE)
#'
#' # Create a MoBi Qualification Report and set the number of simulations to be run per core
#' createQualificationReportMoBi(maxSimulationsPerCore = 8)
#'
#' # Create a MoBi Qualification Report and update Qualification Version Information on title page
#' versionInfo <- QualificationVersionInfo$new("1.1", "2.2","3.3")
#' createQualificationReportMoBi(versionInfo = versionInfo)
#'
createQualificationReportMoBi <- function(qualificationRunnerFolder = "E:/repos/OSPSuite/QualificationRunner/src/QualificationRunner/bin/Debug/net8.0-windows",
                                          mobiPortableFolder = "E:/repos/OSPSuite/SuiteTools/MoBi/src/MoBi.CLI/bin/Debug/net8.0-windows",
  ...
  qualificationPlanName <- "evaluation_plan_mobi.json"
  ...
  #' `ospsuite.reportingengine::startQualificationRunner` does not expose the
  #' `--mobi` flag, so for MoBi plans we invoke QualificationRunner.exe directly
  #' via `system2`. Flags match the PK-Sim path (see `startQualificationRunner`
  #' source), with `--mobi` added and `--pksim` now optional.
  qualificationRunner <- file.path(qualificationRunnerFolder, "QualificationRunner.exe")
  stopifnot(file.exists(qualificationRunner))
  stopifnot(file.exists(qualificationPlanFile))
  stopifnot(dir.exists(mobiPortableFolder))

  if (overwrite && dir.exists(reInputFolder)) {
    unlink(reInputFolder, recursive = TRUE, force = TRUE)
  }

  args <- c(
    "-i", shQuote(qualificationPlanFile),
    "-o", shQuote(reInputFolder),
    "--mobi", shQuote(mobiPortableFolder),
    "--norun",
    if (overwrite) "-f",
    if (!is.null(configurationPlanName)) c("-n", shQuote(configurationPlanName)),
    if (!is.null(pkSimPortableFolder)) c("--pksim", shQuote(pkSimPortableFolder)),
    if (!is.null(logFile)) c("-l", shQuote(logFile)),
    if (!is.null(logLevel)) c("--logLevel", logLevel)
  )

  status <- system2(qualificationRunner, args = args)
  if (status != 0) {
    stop(sprintf("QualificationRunner.exe failed with exit code %d", status))
  }
